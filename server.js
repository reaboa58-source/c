...
function setupCommands() {
  ...
  registerCommand('id', ['ايدي', 'i'], { category: 'عام', description: 'صورة ايدي مع معلومات', async execute(message) { const target = message.mentions.users.first() || message.author; const member = message.guild.members.cache.get(target.id); try { const rankCard = new RankCardBuilder() .setDisplayName(target.username) .setAvatar(target.displayAvatarURL({ extension: 'png', size: 256 })) .setCurrentXP(0) .setRequiredXP(100) .setLevel(0) .setRank(0) .setOverlay(90) .setBackground('#0f0c29') .setProgressBar('#e94560', 'COLOR') .setUsername(target.username, '#ffffff'); const image = await rankCard.build({ format: 'png' }); const attachment = new AttachmentBuilder(image, { name: 'id.png' }); message.reply({ files: [attachment] }); } catch (err) { message.reply(`الايدي: \`${target.id}\``); } } });
